...
arg.tbl = args_set(n = seq(0.03, 0.06, by = 0.005))

# define the plan
plan(sequential) # or could use 'multisession' for parallel processing

# get output as list-column of argument table
results = mutate(arg.tbl, 
  output = future_pmap(arg.tbl, compute_profile,
...
## -----------------------------------------------------------------------------
arg.tbl = args_permute(n = seq(0.03, 0.06, by = 0.005),
