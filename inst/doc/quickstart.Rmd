...
2) Define a plan using `future::plan()` (re-exported by `reval`
for convenience).
3) Evaluate the function and argument sets using `furrr::pmap`
(re-exported by `reval` for convenience).
...
the results (as a list column) to the argument table keeps the outputs
associated with the arguments, allowing you to use the argument values
as identifiers as you do further transformations on the outputs. For
example, `compute_profile()` returns a dataframe which can be unnested
using `tidyr::unnest()` to quickly prepare the data for plotting: 
